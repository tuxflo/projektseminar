...
Entry *init_check_buffer(int world_size) {
    ...
    if(check_buffer == NULL)
        printf("error allocatng space for check buffer!\n");
...
int check_values(LA local_array, int world_size, Entry *check_buffer) {
  ...
  for(i = 0; i < ELEMENT_COUNT * world_size; i++) {
    ...
    if(strncmp(tmp, check_buffer[i].value, strlen(tmp)) != 0 && strcmp(tmp, "COLLISION") != 0 && strcmp(tmp, "NULL") != 0) {
      printf("check buffer %d,  key: %s, value: %s\n", i, check_buffer[i].key, check_buffer[i].value); 
      ret = 1;
    }
  }
  return ret;
